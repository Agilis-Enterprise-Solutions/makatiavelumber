    ...
    def _compute_sale_order_history(self):
        for vals in self:
            ...
            if vals.partner_id:
                ...
                history_domain = []
                if vals.env.company.day:
                    day = vals.env.company.day
                    Display_date = datetime.today() - relativedelta(days=day)
                    domain.append(("date_order", ">=", Display_date),)
                ...
                if vals.env.company.stages:
                    for stage in vals.env.company.stages:
                        if stage.stage_key:
                            stages.append(stage.stage_key)
                    domain.append(("state", "in", stages),)
                    history_domain.append(("status", "in", stages),)

                if vals.env.user.company_id.sh_sale_configuration_limit:
                    limit = vals.env.user.company_id.sh_sale_configuration_limit
                else:
                    limit = None
                ...
                sale_order_search = vals.env["sale.order"].search(
                    domain,
                    limit=limit,
                    order="date_order desc",)
